...
function azure_setup() {
	...
	register_nav_menus(
		array(
			'primary' => __( 'Overlay Menu', 'azure-isle' ),
			'footer'  => __( 'Footer Menu', 'azure-isle' ),
		)
	);
}
...
function azure_assets() {
	wp_enqueue_style( 'azure-fonts', 'https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;1,400&family=Jost:wght@300;400;500&display=swap', array(), null );
...
function azure_body_classes( $classes ) {
	if ( is_front_page() || is_singular( 'azure_room' ) ) {
		$classes[] = 'has-hero';
	}
	return $classes;
}
